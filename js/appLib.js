...
function ajaxCallForTripDates(jsonForTrips){
	...
	j.ajax({
		url : urlPath +"FetchDateForTrips?fe720djlvd="+keyToSend+"&dsfwo82kpo="+passToSend,
		type: 'POST',
		dataType: 'json',
		crossDomain: true,
...
function setDynamicDate(travelRequestDetails){
	...
	/*		var stateArr = new Array();
			...
			var imgAttachment = stateArr.attachment;
			var encodedStr = imgAttachment.replace(/\+/g, '-').replace(/\//g, '_').replace(/\=+$/, '');
			var imgName = stateArr.imgFileName;
			try{
				var s = 
				"<li >"
				...
				+"<tr><td>Image:</td><td><input class='imagePath' data-toggle='modal' data-target='#myModalNew' id ='imgName_"+i+"' value ="+imgName+" onclick='showImageOnModal("+i+",encodedStr);'></td></tr>"    
				+"</table>"     
				+"</div>"
				+"</div>"
				+"</li>";
...
function ajaxCallForTripDetails(jsonForTripDetails){
	urlPath=window.localStorage.getItem("urlPath");
	j.ajax({
		url : urlPath +"FetchTripDetails?fe720djlvd="+keyToSend+"&dsfwo82kpo="+passToSend,
		type: 'POST',
		dataType: 'json',
		crossDomain: true,
...
function ajaxCallForfetchingRequestNos(jsonForRequest){
	urlPath=window.localStorage.getItem("urlPath");
	j.ajax({
		url : urlPath +"FetchRequestNos?fe720djlvd="+keyToSend+"&dsfwo82kpo="+passToSend,
		type: 'POST',
		dataType: 'json',
		crossDomain: true,
...
function ajaxCallForTicketOptions(jsonForTicketOptions){
	urlPath=window.localStorage.getItem("urlPath");
	j.ajax({
		url : urlPath +"FetchTicketOptions?fe720djlvd="+keyToSend+"&dsfwo82kpo="+passToSend,
		type: 'POST',
		dataType: 'json',
		crossDomain: true,
...
function ajaxForSettingPriority(jsonForPreferences){
	urlPath=window.localStorage.getItem("urlPath");
	j.ajax({
		url : urlPath +"SetBookingPriority?fe720djlvd="+keyToSend+"&dsfwo82kpo="+passToSend,
		type: 'POST',
		dataType: 'json',
		crossDomain: true,
...
function showImageOnModal(i,encodedPath){
...
/*	var decodedBase64;
	...
	//decodedBase64 = encodedPath.replace(/-/g, '+').replace(/_/g, '/');
	j('#imgModalBody').empty();
	try{
		var myImages = "<img id='modalImage_"+i+"' class='imageSize'  src='data:image/png;base64,"+encodedPath+"'/>";  		
	}  catch(e){
		alert("exception in dynamic page : " +e);
	}
	j('#imgModalBody').append(myImages);
}
